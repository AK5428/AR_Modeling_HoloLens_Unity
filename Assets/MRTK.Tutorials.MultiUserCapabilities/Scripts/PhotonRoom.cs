...
namespace MRTK.Tutorials.MultiUserCapabilities
{
    ...
    {
        // for dialog
        private DialogCaller dialogCaller = new DialogCaller();
        ...
        private void StartGame()
        {
            ...
            if (!PhotonNetwork.IsMasterClient || PhotonNetwork.CurrentRoom.PlayerCount != 1) {
                // get the players num as string to print in dialog
                string text = "Current room name: " + PhotonNetwork.CurrentRoom.Name + "\n" +
                            "Other players in room: " + PhotonNetwork.CountOfPlayersInRooms + "\n" +
                            "Total players in room: " + (PhotonNetwork.CountOfPlayersInRooms + 1);
                dialogCaller.CallConfirmationDialog(text);
            };


            // Used as the reference to initial objects
